...
MODULE = importlib.import_module('agents.locus_payments')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'locus_payments')
        self.assertEqual(spec.primary_contract_name, 'LocusSpendController')
