...
contract LocusSpendControllerTest is Test {
    LocusSpendController private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_paywithlocus");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new LocusSpendController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        LocusSpendController freshController = new LocusSpendController(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
